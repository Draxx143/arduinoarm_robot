...
| فایل | پلتفرم | نوع |
|---|---|---|
| `AXIS5-Robot-Control-1.0.16-amd64.deb` | Ubuntu / Debian | نصب‌کننده‌ی سیستم |
| `AXIS5-Robot-Control-1.0.16-x86_64.AppImage` | هر لینوکس ۶۴بیتی | قابل‌حمل، بدون نصب |
| `AXIS5-Robot-Control-Setup-1.0.16.exe` | Windows 10/11 | نصب‌کننده |
| `AXIS5-Robot-Control-Portable-1.0.16.exe` | Windows 10/11 | قابل‌حمل، بدون نصب |
...
```bash
# ۱) دانلود آخرین نسخه
wget -c https://github.com/Draxx143/arduinoarm_robot/releases/download/latest/AXIS5-Robot-Control-1.0.16-amd64.deb
...
sha256sum AXIS5-Robot-Control-1.0.16-amd64.deb

# ۳) نصب
sudo apt install ./AXIS5-Robot-Control-1.0.16-amd64.deb
```
...
> اگر فایل را از مرورگر دانلود کردید، فقط مسیر را عوض کنید: `sudo apt install ~/Downloads/AXIS5-Robot-Control-1.0.16-amd64.deb`

### روش ۲ — AppImage (بدون نصب، قابل‌حمل)

```bash
chmod +x AXIS5-Robot-Control-1.0.16-x86_64.AppImage
./AXIS5-Robot-Control-1.0.16-x86_64.AppImage
```

اگر روی توزیع‌های جدید خطای sandbox داد:

```bash
./AXIS5-Robot-Control-1.0.16-x86_64.AppImage --no-sandbox
```
...
1. فایل `AXIS5-Robot-Control-Setup-1.0.16.exe` را از [Releases](https://github.com/Draxx143/arduinoarm_robot/releases/latest) دانلود کنید.
...
فایل `AXIS5-Robot-Control-Portable-1.0.16.exe` را دانلود و مستقیم اجرا کنید — نیازی به نصب ندارد و می‌توانید از فلش هم اجرایش کنید.
