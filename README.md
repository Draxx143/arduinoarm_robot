# 3+1 DOF Robot Arm Firmware for Arduino Mega2560

## Overview

This is a complete firmware for a 3+1-DOF robot arm controlled by an Arduino Mega2560 with ROS integration via rosserial. The firmware handles real-time motor control while ROS handles high-level tasks like inverse kinematics and trajectory planning.
...
- **3+1 Axes Control**: 3 joints + gripper, each with its own motion profile —
  joint 3 is driven by **two face-to-face steppers** (one STEP signal, mirrored DIR)
...
- Arduino Mega2560
- 5 Stepper Motors (NEMA17/NEMA23): one for J1, one for J2, **two face-to-face
  for J3**, one for the gripper
- 5 Stepper Drivers (A4988/TB6600)
- 4 Endstop Switches (NO, NC compatible) — J3's two motors share one endstop
...
|------|------|-----|--------|---------|
| X (J1 base)     | A0   | A1  | 38     | 3       |
| Y (J2 shoulder) | A6   | A7  | A2     | 14      |
| Z (J3 elbow, motor 1) | 46 | 48 | A8  | 18      |
| Z2 (J3, motor 2, mirrored DIR) | 26 | 28 | 24 | — (shared with Z) |
| G (J4 gripper)  | 36   | 34  | 30     | 15      |

> Pin 2 (the old wrist endstop) is **free** now.
...
```
home                         home ALL joints, in priority order J1 -> J2 -> J3 -> J4
home <1-4>                   home a single joint
homeorder                    show the current homing priority
homeorder <j1> <j2> <j3> <j4>
...
moveall <d1> <d2> <d3> <d4>
traj line <d1..d4> <ms>      all axes arrive together in <ms>
demo / stopdemo
enable / disable [<axis>]
estop / reset / stop
...
For every joint, in priority order (`J1 -> J2 -> J3 -> J4` by default):
...
```c
#define HOMING_ORDER         {0, 1, 2, 3}   // priority: joint 1 first
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/Draxx143/arm-3-axis/main/tools/install-linux.sh)
```
...
```bash
wget https://github.com/Draxx143/arm-3-axis/releases/download/latest/AXIS3-Robot-Control-1.0.0-amd64.deb
wget https://github.com/Draxx143/arm-3-axis/releases/download/latest/AXIS3-Robot-Control-1.0.0-x86_64.AppImage
```
...
> صفحه‌ی ریلیز: <https://github.com/Draxx143/arm-3-axis/releases/tag/latest>

اجرا: `axis3-robot-control` (یا از منوی برنامه‌ها). لاگ اجرا: `~/.axis3/last-run.log`.
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/Draxx143/arm-3-axis/main/tools/flash-linux.sh)
```

```bash
bash tools/flash-linux.sh --dry-run             # فقط نشان بده چه می‌کرد
bash tools/flash-linux.sh --port /dev/ttyACM0   # پورتِ دلخواه
bash tools/flash-linux.sh --compile-only        # فقط بساز (.hex در ~/.cache/axis3-build)
...
* **FQBN درست:** `arduino:avr:mega` (Mega 2560). بردِ اصلی معمولاً
  ...
  `/dev/axis3` (نامِ ثابتِ udev) را برمی‌دارد.
...
(ModemManager/brltty + گروه `dialout` + نامِ ثابتِ `/dev/axis3`):

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/Draxx143/arm-3-axis/main/tools/fix-serial-port-ownership.sh)
```
...
می‌دهی، زوایای چهار جوینت حل می‌شود، **اول** با محدوده‌ی درجه‌ی هر جوینت
...
مدل هندسی در GUI و فریم‌ور **یکی** است (ساعد مؤثر `L2+L3`، چون گریپر صفر
...
### نقطه‌ی صفرِ گریپر (جوینت ۴)

`Config.h` → `HOMING_ZERO_OFFSET_DEG {0,0,0,90}`: بعد از هومینگ، گریپر نود
درجه جلو می‌رود و **همان‌جا صفر می‌شود** (پس endstop در ۹۰− درجه است و
دامنه‌ی گریپر قرینه). در GUI هم کنار J4 نوشته شده «صفر ‎+90° از endstop».

### جوینت ۳ دو موتوره است

جوینت ۳ (آرنج) **دو استپر روبروی هم** دارد: هر دو دقیقاً یک پالس STEP
می‌گیرند و DIR موتور دوم همیشه برعکس موتور اول است (`AXIS_Z2_*` در
`Config.h`، پین‌های ۲۶/۲۸/۲۴) تا هر دو هم‌جهت بچرخند و گشتاور دو برابر
شود. تست ۱۸ شبیه‌ساز همین را از روی پین‌ها تأیید می‌کند.
...
| چه دیدی | یعنی | کار |
|---|---|---|
| `AXIS-3 Firmware v1.0.0` + بلوکِ status | پورت، مجوز و فریم‌ور سالم‌اند | مشکل از اپ است؛ اپ را از ترمینال اجرا کن: `axis3-robot-control` |
...
3. **Simulate** the 20 kHz step ISR behaviourally: homing priority and backoff,
   ...
   retargeting, the gripper zero-offset phase, IK/FK sanity, and the dual-motor
   J3 (both steppers get identical STEP pulses, second DIR mirrored) —
   **81 assertions**.
...
11. **Port ownership** (`tools/test_portnames.js`, 48 checks): the phantom
    `/dev/ttyS*` filter, the stable `/dev/axis3` name (substituted in both the
    ...
    that `99-axis3-serial.rules` is really shipped in the package and installed
    by *both* installers, so the fix cannot silently disappear.
12. **Main-process chain end to end** (`tools/test_main_ipc.js`, 29 checks):
...
13. **Browser connect path** (`tools/test_browser_connect.js`, 39 checks): the
    Web Serial path in Chrome/Edge — DTR/RTS reset pulse, boot banner and
    firmware version detection, the silent-board single RESET prompt, garbage
    bytes on a wrong baud, and pulse-sequence parity with the desktop app.

```bash
bash tools/hosttest/run_tests.sh     # همه‌ی سیزده مرحله
cd tools && npm install              # برای مرحله‌های ۷، ۱۰ و ۱۳ (jsdom) — اختیاری
```

مرحله‌های jsdom بدون نصب **SKIP** می‌شوند (نه FAIL)، پس اسکریپت بدون
`npm install` هم کامل اجرا می‌شود.
...
Copyright (c) 2026 Draxx143 — AXIS-3 Robot Arm · https://github.com/Draxx143/arm-3-axis
```
