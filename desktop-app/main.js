...
/* ============================================================
 * main.js — AXIS-3 Robot Control (Electron main process)
...
/* نام‌گذاریِ پورت‌ها: حذفِ ttyS*های شبحی + ترجیحِ نامِ ثابتِ /dev/axis3
...
function listSystemPorts() {
  return new Promise((resolve) => {
    ...
    execFile(cmd, args, { timeout: 4000 }, (err, stdout) => {
      ...
      /* قاعده‌ی udev پروژه (/etc/udev/rules.d/99-axis3-serial.rules) یک
       * symlinkِ ثابت به نامِ /dev/axis3 می‌سازد. با افتِ USB کرنل دستگاه را
...
process.on("uncaughtException", (err) => {
  try {
    ...
    dialog.showErrorBox("AXIS-3 Robot Control — startup error", String((err && err.stack) || err));
  } catch (e) { /* ignore */ }
});
...
/* Safe video mode: AXIS3_SAFE=1 axis3-robot-control  (for VMs / broken GPU drivers) */
if (process.env.AXIS3_SAFE === "1") {
...
function createWindow() {
  win = new BrowserWindow({
    ...
    title: "AXIS-3 · Robot Arm Control",
  ...
  ipcMain.handle("serialport:list", async () => {
    if (!SerialPortC) {
      ...
      try {
        ...
        return { ports: (names || []).map((n) => ({
          path: n,
          friendly: n === portnames.STABLE_NODE ? "AXIS-3 board (stable name)" : "OS serial device",
    ...
    try {
      ...
      /* اگر /dev/axis3 هست، همان را نشانش بده (نه ttyUSB0 که با هر افتِ USB
...
function buildMenu() {
  const template = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: "About AXIS-3 Robot Control",
          click: () => {
            const { dialog } = require("electron");
            dialog.showMessageBox(win, {
              type: "info",
              title: "About",
              message: "AXIS-3 Robot Control v1.0.0",
              detail:
                "Industrial control panel for the 3+1-DOF Arduino Mega 2560 robot arm.\n" +
