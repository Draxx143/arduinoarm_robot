 ...
 *     قاعده‌ی udev پروژه (build/99-axis5-serial.rules) یک symlinkِ ثابت
 *     به نامِ /dev/axis5 می‌سازد؛ اگر وجود داشته باشد، اپ **همان** را باز
 ...
 * COM3، /dev/axis5) نگه داشته می‌شود تا دستگاهِ واقعیِ کسی از فهرست نیفتد.
...
function applyStableName(ports, target, stable) {
  ...
  if (!hit) out.unshift({ path: s, friendly: "AXIS-5 board (stable name)", stable: true });
  return out;
}

/** آیا این نام یک گره‌ی سریالِ واقعی است؟ (برای reconnect و انتخابِ خودکار) */
function isRealSerialNode(name) {
  return /tty(USB|ACM)\d|rfcomm\d|^COM\d+$|^\/dev\/axis5$/i.test(String(name || "").trim());
}
