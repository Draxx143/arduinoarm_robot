...
- Version housekeeping release (the v1.0.15 code shipped under the 1.0.20 number by mistake).
...
## 🆕 What's new in v1.0.20 (firmware audit + GUI fixes)
...
|---|---|---|
| `AXIS5-Robot-Control-Setup-1.0.20.exe` | Windows 10/11 x64 | Run the installer (desktop + start-menu shortcuts) |
| `AXIS5-Robot-Control-Portable-1.0.20.exe` | Windows 10/11 x64 | Single file — just run it, no installation |
| `AXIS5-Robot-Control-1.0.20-amd64.deb` | Ubuntu / Debian | `sudo apt install ./AXIS5-Robot-Control-1.0.20-amd64.deb` |
| `AXIS5-Robot-Control-1.0.20-x86_64.AppImage` | Any Linux x64 | `chmod +x *.AppImage` then run |
...
| OS | Command | Output |
|---|---|---|
| Windows | `npm run dist:win` | `dist/AXIS5-Robot-Control-Setup-1.0.20.exe` (installer) + `AXIS5-Robot-Control-Portable-1.0.20.exe` (portable) |
