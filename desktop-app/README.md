# AXIS-3 Robot Control — Desktop App

Industrial English-language **desktop application** (Electron) for the 3+1-DOF Arduino Mega 2560 robot arm firmware (`RobotArm_Firmware.ino`).
...
- **Firmware version gate (re-flash guidance built in)** — the board now prints `AXIS-3 Firmware v1.0.38` at boot and `FW: v1.0.38` in every status block. Both apps parse it and show a prominent warning + toast when the connected board runs an older build: *"the E-STOP / smooth-J2 / homing fixes are NOT on this board — flash firmware/RobotArm_Firmware/ then reconnect"*. No more silent "I updated the app but the board still misbehaves".
...
- **Homing order J1 → J2 → J3 → J4** with the **J2/J3 choreography** — J2 nudges forward 250 steps, holds while J3 homes, then returns exactly to its back-off position.
...
**https://github.com/Draxx143/arm-3-axis/releases/latest**

| File | Platform | How to install |
|---|---|---|
| `AXIS3-Robot-Control-Setup-1.0.31.exe` | Windows 10/11 x64 | Run the installer (desktop + start-menu shortcuts) |
| `AXIS3-Robot-Control-Portable-1.0.31.exe` | Windows 10/11 x64 | Single file — just run it, no installation |
| `AXIS3-Robot-Control-1.0.31-amd64.deb` | Ubuntu / Debian | `sudo apt install ./AXIS3-Robot-Control-1.0.31-amd64.deb` |
| `AXIS3-Robot-Control-1.0.31-x86_64.AppImage` | Any Linux x64 | `chmod +x *.AppImage` then run |
...
| OS | Command | Output |
|---|---|---|
| Windows | `npm run dist:win` | `dist/AXIS3-Robot-Control-Setup-1.0.31.exe` (installer) + `AXIS3-Robot-Control-Portable-1.0.31.exe` (portable) |
| Linux | `npm run dist:linux` | `dist/AXIS3-Robot-Control-1.0.1-x64.AppImage` + `.deb` |
...
| Symptom | Fix |
|---|---|
| App dies instantly from the menu icon | Use the terminal launcher `axis3-robot-control`, then check `~/.axis3/last-run.log` |
...
| Garbled/blank window in a VM | Run `AXIS3_SAFE=1 axis3-robot-control` (disables GPU accel) |
