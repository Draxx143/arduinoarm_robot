...
- **Duplicate send fixed** — a single slider release sent the command twice (leading + trailing carried the same value). Now a single release sends **exactly once**; the trailing send only fires when the value moved on after the leading one.
- **Split-line "Unknown command" fixed (re-flash the firmware)** — USB can deliver a command's bytes in chunks with >150 ms gaps while the board is busy; the old 50 ms idle-flush then executed half lines like `de` → *"Unknown command"*. The idle window is now 150 ms and fragments shorter than 3 chars are never executed (no firmware command is shorter). Verified by host tests.

## 🆕 What's new in v1.0.31

- **Fixed-viewport app** — the window no longer scrolls as a page: header, tab bar and footer stay pinned, only the tab's own content scrolls inside its column when needed.
- **Edge-to-edge layout** — the 1620 px width cap is gone; on wide screens the app uses the whole window and the terminal column grows with it (up to ~900 px wide), so the serial console shows far more text per line.

## 🆕 What's new in v1.0.31

- **Link Status card removed**; the console toolbar next to 🔬 Port Test now shows only **TX / RX volumes** (`TX 1.2 kB · RX 340 B`).
- **The serial console now owns the whole right sidebar** — it stretches to the full window height, so every line is visible without scrolling the sidebar (fixes clipped text).
- **Quick chips moved below the terminal** (under the input line).
- **E-STOP / RESET / INITIALIZING buttons removed** — keyboard shortcuts instead: **Esc** = E-STOP, **F4** = RESET (clears E-STOP), **F2** = INIT (⌂ Home All); the header still shows RUN/COM/FAULT lamps and the footer lists all shortcuts.
...
- Version housekeeping release (the v1.0.15 code shipped under the 1.0.31 number by mistake).
...
## 🆕 What's new in v1.0.31 (firmware audit + GUI fixes)
