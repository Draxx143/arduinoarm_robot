...
const FW = {
  ...
  /* Default homing order is J1->J4 (changeable at runtime with homeorder) */
  ...
  DEMO_MOVES: [
    { label: "Pose 1 — Neutral", angles: [0, 0, 0, 0] },
    ...
    { label: "Pose 6 — Return",  angles: [0, 0, 0, 0] },
  ],
  DEMO_MAX_REPEATS: 3,
  DEMO_DELAY_MS: 1500,
  ...
  AXES: [
    ...
    {
      id: "Z", joint: 3, name: "Elbow", role: "Pitch — bends the forearm (dual opposed motors)",
      /* ---- generated from firmware Config.h by tools/sync_gui_config.py ---- */
      min: 0, max: 55,
    ...
    {
      id: "G", joint: 4, name: "Gripper", role: "Open/close — tool rotation",
      /* ---- generated from firmware Config.h by tools/sync_gui_config.py ---- */
      min: -90, max: 90,
...
const Kin = {
  ...
  /** Forward kinematics, identical to the firmware solveFK(): [base,shoulder,elbow,gripper] -> xyz (mm) */
  fk(angles) {
    ...
    const dir = a1 - a2;   /* tool continues the forearm (the gripper does not bend) */
  ...
  /** Inverse kinematics, identical to the firmware solveIK(): xyz -> angles (gripper zeroed) */
  ik(x, y, z) {
    const { L1, L2, L3 } = FW.LINKS;
    ...
    angles[3] = 0;   /* IK leaves the gripper at zero */
    return angles;
  },
...
const Cmd = {
  ...
  /* firmware format: timer <ms> <axis 1-4> <degrees>
     NOTE: degrees is MANDATORY. With only two args the firmware defaults the
     target to 0.0 deg and silently drives that joint to zero. */
...
const Parse = {
  ...
  /* v1.0.36: متن واقعی فریمور بعد از profile <x> */
...
const COMMAND_REF = [
  { cmd: "home", args: "", desc: "Smart homing of all axes (order J1 to J4)", cat: "Basic" },
  { cmd: "home", args: "<1-4>", desc: "Smart homing of a single axis", cat: "Basic" },
  { cmd: "status", args: "", desc: "Full system + axes status report", cat: "Basic" },
  { cmd: "enable", args: "", desc: "Energize all stepper motors", cat: "Basic" },
  { cmd: "enable", args: "<1-4>", desc: "Energize one axis", cat: "Basic" },
  { cmd: "disable", args: "", desc: "De-energize all motors", cat: "Basic" },
  { cmd: "disable", args: "<1-4>", desc: "De-energize one axis", cat: "Basic" },
  ...
  { cmd: "fk", args: "<a1>..<a4>", desc: "Forward kinematics: angles -> XYZ", cat: "Kinematics" },
