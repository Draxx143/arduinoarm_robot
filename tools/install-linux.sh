...
# ======================================================================
#  AXIS-3 Robot Control — نصب/پاک‌سازی کامل از ترمینال لینوکس
#
...
#    ۵. پورت را از دستِ ModemManager و brltty آزاد می‌کند + نامِ ثابتِ /dev/axis3
#    ۵. نتیجه را راستی‌آزمایی می‌کند و دستور اجرا/لاگ را چاپ می‌کند
# ======================================================================
set -uo pipefail

REPO="${AXIS3_REPO:-Draxx143/arm-3-axis}"
TAG="${AXIS3_TAG:-latest}"
PKG="axis3-robot-control"
WORKDIR="${HOME}/.cache/axis3-installer"
...
mkdir -p "$WORKDIR" 2>/dev/null || WORKDIR="/tmp/axis3-installer"
...
for cand in "$(dirname "${BASH_SOURCE[0]}")/fix-serial-port-ownership.sh" \
            "${HOME}/.cache/axis3-installer/fix-serial-port-ownership.sh"; do
    [ -f "$cand" ] && { FIXER="$cand"; break; }
done
if [ -z "$FIXER" ]; then
    mkdir -p "${HOME}/.cache/axis3-installer" 2>/dev/null || true
    FIXER="${HOME}/.cache/axis3-installer/fix-serial-port-ownership.sh"
    if command -v curl >/dev/null 2>&1; then
        curl -fsSL "https://raw.githubusercontent.com/${REPO}/${AXIS3_BRANCH:-main}/tools/fix-serial-port-ownership.sh" -o "$FIXER" 2>/dev/null || FIXER=""
    elif command -v wget >/dev/null 2>&1; then
        wget -q "https://raw.githubusercontent.com/${REPO}/${AXIS3_BRANCH:-main}/tools/fix-serial-port-ownership.sh" -O "$FIXER" 2>/dev/null || FIXER=""
    fi
fi
if [ -n "$FIXER" ] && [ -f "$FIXER" ]; then
    ...
    c_y "  اسکریپتِ آزادسازی پیدا نشد — دستی: desktop-app/build/99-axis3-serial.rules را در /etc/udev/rules.d/ کپی کن"
fi
...
if [ "$MODE" = "deb" ]; then
    ...
    command -v axis3-robot-control >/dev/null 2>&1 && c_g "  لانچر: $(command -v axis3-robot-control)"
else
...
────────────────────────────────────────────────────────────────
 اجرا
────────────────────────────────────────────────────────────────
   از منوی برنامه‌ها:  AXIS3 Robot Control
   یا از ترمینال:      axis3-robot-control
   (AppImage)         ~/.local/bin/AXIS3-Robot-Control-*.AppImage

 لاگ اجرا (وقتی چیزی کار نمی‌کند، اول این را ببین):
   cat ~/.axis3/last-run.log
