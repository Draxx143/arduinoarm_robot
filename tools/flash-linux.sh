...
# ======================================================================
#  AXIS-3 — فلشِ کاملِ فریم‌ور روی Mega 2560 از ترمینالِ اوبونتو
#
...
REPO_BRANCH="main"
FQBN="${AXIS3_FQBN:-arduino:avr:mega}"
BAUD="115200"
DRY=0
COMPILE_ONLY=0
KEEP_ROS=0
PORT="${AXIS3_PORT:-}"
CLI="${AXIS3_ARDUINO_CLI:-}"
BUILD_DIR="${AXIS3_BUILD:-$HOME/.cache/axis3-build}"
...
find_sketch() {
    ...
    for d in "$here/../firmware/RobotArm_Firmware" "./firmware/RobotArm_Firmware" \
             "$HOME/arm-3-axis/firmware/RobotArm_Firmware"; do
...
SKETCH="$(find_sketch)" || {
    ...
    info "git clone https://github.com/Draxx143/arm-3-axis.git && cd arm-3-axis"
...
if [ -n "$CLI" ]; then
    ...
    run "mkdir -p \"\$HOME/.local/bin\" \"\$HOME/.cache/axis3-cli\""
    run "curl -fsSL https://raw.githubusercontent.com/arduino/arduino-cli/master/install.sh -o \"\$HOME/.cache/axis3-cli/install.sh\""
    run "BINDIR=\"\$HOME/.local/bin\" sh \"\$HOME/.cache/axis3-cli/install.sh\""
...
step "۴) پورت"
if [ -z "$PORT" ]; then
    for p in /dev/axis3 /dev/ttyACM0 /dev/ttyACM1 /dev/ttyUSB0 /dev/ttyUSB1; do
        [ -e "$p" ] && { PORT="$p"; break; }
    done
fi
if [ -z "$PORT" ]; then
    bad "هیچ پورتِ سریالی پیدا نشد (/dev/axis3، /dev/ttyACM*، /dev/ttyUSB*)"
...
info "باید بنرِ «AXIS-3 Firmware v${FW_VER:-?}» را ببینی. اگر «پورت دستِ برنامه‌ی دیگری است» گفت:"
info "  bash <(curl -fsSL https://raw.githubusercontent.com/Draxx143/arm-3-axis/${REPO_BRANCH}/tools/fix-serial-port-ownership.sh)"
