...
echo "════════════════════════════════════════════════════════════"
echo "  AXIS-3 — عیب‌یابیِ اتصالِ برد        $(date '+%Y-%m-%d %H:%M:%S')"
echo "════════════════════════════════════════════════════════════"
...
# ---------------------------------------------------------------- ۲ اپ
hr "۲) اپِ نصب‌شده"
if command -v axis3-robot-control >/dev/null 2>&1; then
    ok "axis3-robot-control پیدا شد: $(command -v axis3-robot-control)"
    dpkg -l 2>/dev/null | awk '/axis3-robot-control/{print "  · نسخه‌ی deb: "$3}'
else
...
if [ -f /etc/udev/rules.d/99-axis3-serial.rules ]; then
    ok "قاعده‌ی udev پروژه نصب است (ModemManager چشم می‌پوشد + نامِ ثابت)"
else
    bad "قاعده‌ی udev پروژه نصب نیست: /etc/udev/rules.d/99-axis3-serial.rules"
    OWNERSHIP_BAD=1
fi
if [ -e /dev/axis3 ]; then
    ok "/dev/axis3 → $(readlink -f /dev/axis3 2>/dev/null) (نامِ ثابت؛ با افتِ USB عوض نمی‌شود)"
else
    info "/dev/axis3 وجود ندارد — بعد از نصبِ قاعده یک بار کابل را بکش و دوباره بزن"
fi
if [ "$OWNERSHIP_BAD" = 1 ]; then
    echo
    bad "ریشه‌ی «دستور می‌رود ولی جواب برنمی‌گردد» همین‌جاست. یک دستور درستش می‌کند:"
    echo "      bash <(curl -fsSL https://raw.githubusercontent.com/Draxx143/arm-3-axis/main/tools/fix-serial-port-ownership.sh)"
...
print("\n  ── جمع‌بندی ──")
if "AXIS-3 Firmware v" in txt or "FW: v" in txt:
    import re
    ...
    print("      اپ را از ترمینال اجرا کن تا خطاهایش دیده شود:  axis3-robot-control")
    ...
    print("    firmware/RobotArm_Firmware/ را دوباره فلش کن (باید v1.0.0 باشد).")
...
echo "        axis3-robot-control        (یا:  AppImage را از ترمینال اجرا کن)"
