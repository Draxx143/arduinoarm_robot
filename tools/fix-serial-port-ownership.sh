...
#       (ID_MM_DEVICE_IGNORE + مجوزِ dialout + نامِ ثابتِ /dev/axis3)
...
RULE_NAME="99-axis3-serial.rules"
RULE_DST="/etc/udev/rules.d/$RULE_NAME"
BRLTTY_OVERRIDE="/etc/udev/rules.d/85-brltty.rules"
RAW_URL="https://raw.githubusercontent.com/Draxx143/arm-3-axis/main/desktop-app/build/$RULE_NAME"
MODE="install"
case "${1:-}" in
  --check) MODE="check" ;;
  --undo)  MODE="undo" ;;
...
find_rule() {
  ...
  for cand in \
      ...
      "$HOME/.cache/axis3-installer/$RULE_NAME"; do
...
report_state() {
  ...
  if [ -e /dev/axis3 ]; then
    ok "/dev/axis3 وجود دارد → $(readlink -f /dev/axis3 2>/dev/null || echo '?')"
    info "این نام با افتِ USB عوض نمی‌شود (برخلافِ ttyUSB0/ttyUSB1)"
  else
    bad "/dev/axis3 وجود ندارد (قاعده نصب نشده، یا کابل بعد از نصب وصل نشده)"
  fi
...
if [ -n "$BRL_SRC" ]; then
  ...
  tmp="$(mktemp)"
  sed -E '/1a86/ s/^/#AXIS3: /' "$BRL_SRC" > "$tmp"
...
echo
if [ -e /dev/axis3 ]; then
  c_g "آماده است: /dev/axis3 → $(readlink -f /dev/axis3)"
else
  c_y "یک بار کابلِ USB را بکش و دوباره بزن تا /dev/axis3 ساخته شود"
fi
echo
report_state
