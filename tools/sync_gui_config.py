...
AXIS_IDS = ["X", "Y", "Z", "G"]               # ایندکس ۰..۳ = جوینت ۱..۳ + گریپر
# نکته: موتور دوم جوینت ۳ (Z2، پین‌های ۲۶/۲۸/۲۴، آینه‌ای) سرعت/محدوده‌ی
# جداگانه ندارد و از AXIS_Z پیروی می‌کند؛ پس در این فهرست نیست.
...
def firmware_scalars():
    ...
    order = [int(x) for x in m.group(1).replace(" ", "").split(",") if x != ""] if m else [0, 1, 2, 3]
