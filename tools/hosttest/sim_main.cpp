...
//
//  تمرکز این فایل: اولویت هومینگ (J1 -> J4)، بک‌آف اجباری و تأیید
...
static const int32_t AXIS_BACKOFF[NUM_AXES] = {
    AXIS_X_BACKOFF, AXIS_Y_BACKOFF, AXIS_Z_BACKOFF, AXIS_G_BACKOFF
};
...
static uint64_t g_stepsM2;                  // استپ‌های موتور دوم جوینت ۳ (پین Z2)
    ...
    for (int i = 0; i < NUM_AXES; i++) {
        ...
        if (a->tick()) {
            ...
            if (sim_read_output(STEP_PIN[i])) {
                g_steps[i]++;
                // جوینت ۳ (i=2) دو موتور روبروی هم دارد: موتور دوم باید
                // دقیقاً هم‌زمان با موتور اول پالس بگیرد
                if (i == 2 && sim_read_output(AXIS_Z2_STEP_PIN)) g_stepsM2++;
...
static void runHomingWithOrder(double maxSeconds = 200.0) {
    ...
    for (uint64_t i = 0; i < n; i++) {
        ...
        // (برای J4 = ۹۰ درجه جلو رفتن و صفر شدن) در جریان است. اگر اینجا
...
int main() {
    ...
    // ------------------------------------------------------------------
    header("تست ۱: ترتیب اولویت هومینگ = جوینت ۱، ۲، ۳، ۴");
    // ------------------------------------------------------------------
    freshStart();
    ...
    check(order2, "ترتیب در هومینگ دوم هم J1 -> J4 بود");
    bool bo2 = true;
    for (int i = 0; i < NUM_AXES; i++) if (g_backoffSteps[i] == 0) bo2 = false;
    check(bo2, "در هومینگ دوم هم بک‌آف برای همه انجام شد");

    // ------------------------------------------------------------------
    header("تست ۶: تغییر ترتیب در زمان اجرا (homeorder 3 2 1 4)");
    // ------------------------------------------------------------------
    freshStart();
    uint8_t custom[NUM_AXES] = {2, 1, 0, 3};
    ...
    check(orderCustom, "ترتیب اجرا مطابق ترتیب تنظیم‌شده بود (J3 -> J2 -> J1 -> J4)");

    // ترتیب نامعتبر باید رد شود
    uint8_t bad1[NUM_AXES] = {0, 0, 1, 2};        // تکراری
    uint8_t bad2[NUM_AXES] = {0, 1, 2, 9};        // خارج از محدوده
    ...
    // ------------------------------------------------------------------
    header("تست ۱۵: آفست نقطه‌ی صفر گریپر (J4) — بعد از هوم ۹۰° جلو، همان‌جا صفر");
    // ------------------------------------------------------------------
    // Config.h: HOMING_ZERO_OFFSET_DEG = {0,0,0,90}
    // یعنی گریپر (جوینت ۴) بعد از هومینگ ۹۰ درجه جلو می‌رود و آنجا نقطه‌ی
    // صفرش می‌شود؛ در نتیجه endstop دقیقاً در ۹۰- درجه می‌نشیند و دامنه‌ی
    // گریپر قرینه است.
    {
        ...
        for (int i = 0; i < NUM_AXES; i++) g_switchAt[i] = -2000 - 300 * i;
        runHomingWithOrder();
        check(g_doneCount == NUM_AXES, "هومینگ کامل شد (۴ جوینت)");
        check(!globalController->isHoming(),
              "توالی هومینگ بعد از فاز آفست بسته شد");

        Axis* a4 = globalController->getAxis(3);
        float spdG = a4->getStepsPerDegree();
        int32_t wantOff = (int32_t)(ZOD[3] * spdG + 0.5f);
        uint64_t extraG = g_steps[3] - g_searchSteps[3] - g_backoffSteps[3];
        printf("   J4: stepsPerDeg=%.2f  استپِ آفست=%llu  انتظار=%d\n",
               spdG, (unsigned long long)extraG, wantOff);
        check(a4->getCurrentPosition() == 0,
              "گریپر بعد از آفست دقیقاً روی صفرِ جدید است");
        check((int32_t)extraG == wantOff,
              "گریپر واقعاً ۹۰ درجه جلو رفت (نه فقط شمارنده صفر شد)");
        ...
        check(othersClean, "جوینت‌های ۱ تا ۳ آفست نگرفتند (فقط گریپر)");

        // ۹۰- درجه روی گریپر باید دقیقاً همان نقطه‌ی endstopِ قدیمی باشد
        a4->moveTo(-wantOff);
        runFor(8.0);
        printf("   J4 بعد از حرکت به ۹۰- درجه: %d استپ\n", (int)a4->getCurrentPosition());
        check(a4->getCurrentPosition() == -wantOff,
              "۹۰- درجه = نقطه‌ی endstop قبلی (صفرِ جدید ۹۰° جلوتر است)");
        check(!a4->homingFailed(), "آفست صفر باعث خطای هومینگ نشد");
    }

    // ------------------------------------------------------------------
    header("تست ۱۶: home 4 (تک‌محوری) هم آفست صفر می‌گیرد");
    // ------------------------------------------------------------------
    {
        static const float ZOD[NUM_AXES] = HOMING_ZERO_OFFSET_DEG;
        freshStart();
        for (int i = 0; i < NUM_AXES; i++) g_switchAt[i] = -2000 - 300 * i;
        resetCounters();
        globalController->smartHomingAxis(3);
        runFor(90.0);
        Axis* a4 = globalController->getAxis(3);
        int32_t wantOff = (int32_t)(ZOD[3] * a4->getStepsPerDegree() + 0.5f);
        uint64_t extraG = g_steps[3] - g_searchSteps[3] - g_backoffSteps[3];
        printf("   home 4: استپِ آفست=%llu  انتظار=%d  موقعیت=%d\n",
               (unsigned long long)extraG, wantOff, (int)a4->getCurrentPosition());
        check(!globalController->isHoming(), "هوم تک‌محوری گریپر تمام شد");
        check(a4->isHomed(), "گریپر هوم‌شده علامت خورد");
        check(a4->getCurrentPosition() == 0, "home 4 هم روی صفرِ جدید نشست");
        check((int32_t)extraG == wantOff, "home 4 هم ۹۰ درجه جلو رفت");
    }
    ...
    // ------------------------------------------------------------------
    {
        ...
        if (solved) {
            static const float DMIN[NUM_AXES] = {-110.0f, 0.0f, 0.0f, -90.0f};
            static const float DMAX[NUM_AXES] = { 110.0f, 100.0f, 55.0f, 90.0f};
            bool inRange = true;
            for (int i = 0; i < NUM_AXES; i++) {
    ...
    // ------------------------------------------------------------------
    header("تست ۱۸: جوینت ۳ دو موتوره — STEP هم‌زمان + DIR آینه‌ای");
    // ------------------------------------------------------------------
    // هر دو موتور جوینت ۳ باید دقیقاً یک پالس بگیرند و DIR موتور دوم
    // همیشه برعکس موتور اول باشد (نصب روبروی هم).
    {
        freshStart();
        Axis* j3 = globalController->getAxis(2);
        globalController->smartHomingAxis(2);
        runFor(90.0);
        check(j3->isHomed(), "جوینت ۳ هوم شد");
        const int32_t TARGET = 3000;
        j3->moveTo(TARGET);
        double t0 = elapsed();
        while (j3->isMoving() && (elapsed() - t0) < 30.0) isr_tick();
        check(j3->getCurrentPosition() == TARGET, "جوینت ۳ به هدف رسید");
        printf("   موتور اول=%llu استپ  موتور دوم=%llu استپ\n",
               (unsigned long long)g_steps[2], (unsigned long long)g_stepsM2);
        check(g_stepsM2 == g_steps[2] && g_stepsM2 > 0,
              "موتور دوم دقیقاً به تعداد موتور اول پالس گرفت");
        check(sim_read_output(AXIS_Z_DIR_PIN) != sim_read_output(AXIS_Z2_DIR_PIN),
              "DIR موتور دوم برعکس موتور اول است (روبروی هم)");
        // جهت منفی هم باید آینه بماند
        j3->moveTo(0);
        t0 = elapsed();
        while (j3->isMoving() && (elapsed() - t0) < 30.0) isr_tick();
        check(j3->getCurrentPosition() == 0, "جوینت ۳ به صفر برگشت");
        check(sim_read_output(AXIS_Z_DIR_PIN) != sim_read_output(AXIS_Z2_DIR_PIN),
              "آینه‌ای بودن DIR در جهت منفی هم برقرار است");
    }
