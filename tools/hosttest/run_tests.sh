...
if [ $FAIL -eq 0 ] && command -v node >/dev/null 2>&1 && command -v python3 >/dev/null 2>&1; then
    echo
    echo "===== [9/11] connection doctor (fake board on a pty) ====="
...
if [ $FAIL -eq 0 ] && command -v python3 >/dev/null 2>&1; then
    echo
    echo "===== [10/11] reset pulse leaves the board RUNNING (DTR==RTS) ====="
...
if [ $FAIL -eq 0 ] && command -v node >/dev/null 2>&1; then
    echo
    echo "===== [11/11] connect paths in a real DOM (Web Serial lines, IPC default, baud ladder) ====="
