...
if [ $FAIL -eq 0 ] && command -v node >/dev/null 2>&1 && command -v python3 >/dev/null 2>&1; then
    echo
    echo "===== [9/12] connection doctor (fake board on a pty) ====="
...
# ---------------------------------------------------------------------
# ۱۰) پالسِ ریستِ برد — ریشه‌ی «پورت شناسایی می‌شود ولی RX=0 می‌ماند».
...
if [ $FAIL -eq 0 ] && command -v python3 >/dev/null 2>&1; then
    echo
    echo "===== [10/12] reset pulse leaves the board RUNNING (DTR==RTS) ====="
...
# ---------------------------------------------------------------------
# ۱۱) مسیرهای اتصالِ رندر — سه علتِ واقعیِ «شناسایی می‌شود ولی RX=0»:
#     پالسِ خطوط در وب‌سریال، پیش‌فرض شدنِ پلِ سیستمی با کشوی خالی، و
#     بازیابیِ خودکار با نردبانِ baud. با electronAPIِ ساختگی در jsdom.
# ---------------------------------------------------------------------
if [ $FAIL -eq 0 ] && command -v node >/dev/null 2>&1; then
    echo
    echo "===== [11/12] connect paths in a real DOM (Web Serial lines, IPC default, baud ladder) ====="
...
# ---------------------------------------------------------------------
# ۱۲) «برد را پیدا کن» — وقتی همه‌چیز سبز است ولی یک بایت هم نمی‌آید، باید
#     هر گره‌ی واقعی را در هر سرعتِ محتمل امتحان کند و بگوید برد کجاست
#     (ttyUSB1 نه ttyUSB0) و پورت‌های شبحیِ ttyS را از فهرست بیرون کند.
# ---------------------------------------------------------------------
if [ $FAIL -eq 0 ] && command -v node >/dev/null 2>&1 && command -v python3 >/dev/null 2>&1; then
    echo
    echo "===== [12/12] find-the-board scanner (fake board, phantom ttyS filtering) ====="
    printf "  %-34s " "scan.js: port × baud sweep"
    if node "$ROOT/tools/test_scan.js" > "$OUT/scan.log" 2>&1; then
        echo "OK — $(grep -m1 'نتیجه:' "$OUT/scan.log")"
    else
        echo "FAIL"; sed 's/^/      /' "$OUT/scan.log" | tail -22; FAIL=1
    fi
fi
