...
async function testOneGui(label, port, page, lang) {
  ...
  const T = lang === "fa"
    ? { stray: "ردیفِ اسلایدر گره‌ی متنیِ ولگرد ندارد (کامنتِ جاافتاده → ستونِ ضمنیِ گرید)",
        ...
        j4: "ردیفِ J4 (گریپر) آفستِ صفرِ ۹۰+ درجه را نشان می‌دهد",
    ...
    : { stray: "slider row has no stray text node (a misplaced comment becomes an implicit grid column)",
        ...
        j4: "the J4 (gripper) row shows the +90° zero offset",
  ...
  ok(rows.length === 4, `${label}: ۴ ردیفِ اسلایدر ساخته شد`);
  ...
  ok(!jval, T.nojval);
  ok(/\+\s?90|90°/.test(rows[3].textContent), T.j4);
  ...
  await sleep(900);
  txt = consoleText(dom);
  ok(count(txt, /System Status/g) === 1 && count(txt, /Axis \d:/g) >= 4 &&
