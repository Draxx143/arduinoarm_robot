 ...
 *   ۱) نام‌گذاریِ پورت: حذفِ ttyS*های شبحی + ترجیحِ نامِ ثابتِ /dev/axis5
...
ok(portnames.isRealSerialNode("/dev/ttyUSB0") && portnames.isRealSerialNode("/dev/axis5")
   && !portnames.isRealSerialNode("/dev/ttyS7") && !portnames.isRealSerialNode(""),
   "isRealSerialNode: گره‌ی واقعی، نامِ ثابت، و ردِ ttyS");

/* ---------- ۲) نامِ ثابتِ /dev/axis5 ---------- */
console.log("\n-- نامِ ثابت: با افتِ USB عوض نمی‌شود --");
const fakeFs = { existsSync: (p) => p === "/dev/axis5", realpathSync: (p) => "/dev/ttyUSB1" };
...
ok(swapped.indexOf("/dev/axis5") !== -1 && swapped.indexOf("/dev/ttyUSB1") === -1,
...
ok(renamed[0].path === "/dev/axis5" && renamed[0].vid === "1a86",
...
ok(portnames.applyStableName([], target)[0].path === "/dev/axis5",
...
(async () => {
  ...
  ok(fs.existsSync(rulePath), "فایلِ قاعده وجود دارد: desktop-app/build/99-axis5-serial.rules");
  ...
  ok(/SYMLINK\+="axis5"/.test(rule), "نامِ ثابتِ /dev/axis5 ساخته می‌شود");
  ...
  ok(/99-axis5-serial\.rules/.test(extra), "قاعده در بسته‌ی اپ جاسازی می‌شود (extraFiles)");
  ...
  ok(/\/etc\/udev\/rules\.d\/99-axis5-serial\.rules/.test(after),
