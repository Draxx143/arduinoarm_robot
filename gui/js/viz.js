...
class ArmViz {
  constructor(canvas) {
    ...
    this.current = [0, 0, 0, 0];  // زوایای فعلی (درجه) — با lerp نرم می‌رسد
    this.target = [0, 0, 0, 0];   // زوایای هدف (شبح)
  ...
  setAngles(deg4) { this.current = deg4.slice(); }
  setTargets(deg4) { this.target = deg4.slice(); }
  ...
  geometry(angles) {
    ...
    const a0 = angles[0] * D, a1 = angles[1] * D, a2 = angles[2] * D;
    const j2 = { x: 0, z: 0 };                                     // شانه (مبدا)
    const j3 = { x: L1 * Math.cos(a1), z: L1 * Math.sin(a1) };      // آرنج
    const j4 = { x: j3.x + L2 * Math.cos(a1 - a2), z: j3.z + L2 * Math.sin(a1 - a2) }; // سر ساعد
    const dir = a1 - a2;   // ابزار در امتداد ساعد است (گریپر خم ندارد)
    const tip = { x: j4.x + L3 * Math.cos(dir), z: j4.z + L3 * Math.sin(dir) };  // نوک
    return { j2, j3, j4, tip, r: Math.hypot(tip.x, tip.z), yaw: angles[0], grip: angles[3] };
  }
  ...
  _drawArm(g, P, s, opts) {
    ...
    if (!opts.ghost) {
      ...
      /* گیره */
      this._drawGripper(j4, tip, g.grip, opts);
    }
    ctx.restore();
  ...
  _drawGripper(j4, tip, gripDeg, opts) {
    const ctx = this.ctx;
    const ang = Math.atan2(tip.y - j4.y, tip.x - j4.x);
    const roll = gripDeg * Math.PI / 180 * 0.35;
    ctx.save();
  ...
  _drawTop(x0, pw) {
    ...
    ctx.fillText("J4: " + g.grip.toFixed(0) + "°", x0 + 24, h - 12);
