...
const FW = {
  ...
  versionOk(reported) {
    ...
    const a = parse(reported), b = parse(this.EXPECTED_FW);
    for (let i = 0; i < 3; i++) {
  ...
  /* ترتیب پیش‌فرض هومینگ J1→J4 است (با homeorder در زمان اجرا عوض می‌شود) */
  HOMING_ORDER: [0, 1, 2, 3],   /* v1.0.35: J1 -> J2 -> J3 -> J4 (sync with firmware) */
  HOMING_ORDER_LABEL: "J1 → J2 → J3 → J4",
  ...
  DEMO_MOVES: [
    { label: "Pose 1 — خنثی",   angles: [0, 0, 0, 0] },
    { label: "Pose 2",          angles: [45, 30, 20, 10] },
    { label: "Pose 3",          angles: [-45, 50, 40, -10] },
    { label: "Pose 4",          angles: [30, 80, 50, 20] },
    { label: "Pose 5",          angles: [-30, 20, 10, -20] },
    { label: "Pose 6 — بازگشت", angles: [0, 0, 0, 0] },
  ],
  DEMO_MAX_REPEATS: 3,
  DEMO_DELAY_MS: 1500,
  ...
  AXES: [
    ...
    {
      id: "Z", joint: 3, name: "آرنج", nameEn: "Elbow", role: "Pitch (خم ساعد) — دو موتور روبروی هم",
      /* ---- generated from firmware Config.h by tools/sync_gui_config.py ---- */
      min: 0, max: 55,
      ...
      soft: { min: 0, max: 3911 },
      /* ---- end generated ---- */
      pins: { step: "46", dir: "48", enable: "A8", endstop: "18", step2: "26", dir2: "28", enable2: "24" },
    },
    {
      id: "G", joint: 4, name: "گریپر", nameEn: "Gripper", role: "باز/بسته (چرخش ابزار)",
      /* ---- generated from firmware Config.h by tools/sync_gui_config.py ---- */
      min: -90, max: 90,
...
const Kin = {
  ...
  /** FK دقیقاً مطابق solveFK فریم‌ور: زوایای [پایه، شانه، آرنج، گریپر] → xyz (mm) */
  fk(angles) {
    ...
    /* امتداد ابزار در ادامه‌ی ساعد است (گریپر خم ندارد) */
  ...
  /** IK مطابق solveIK فریم‌ور: xyz → زوایا (فقط ۳ جوینت اول، گریپر صفر) */
  ik(x, y, z) {
    const { L1, L2, L3 } = FW.LINKS;
    ...
    angles[3] = 0;   /* گریپر در حل IK صفر می‌ماند */
    return angles;
  },
...
const Cmd = {
  ...
  homeAxis: (n) => `home ${n}`,
  status: () => "status",
  /* کانال همگام‌سازی موقعیت: یک خط «>> POS j1,..,j4» برمی‌گرداند و echo
  ...
  stopDemo: () => "stopdemo",
  stop: () => "stop",
  moveAll: (deg4) => `moveall ${deg4.map((d) =>Fmt.num(d)).join(" ")}`,
  ...
  /* قالب فریم‌ور: timer <ms> <axis 1-4> <degrees>
     درجه اجباری است! با دو آرگومان، فریم‌ور هدف را ۰ درجه فرض می‌کند و
     بی‌سروصدا آن محور را به صفر می‌برد. */
  timer: (ms, axis, deg) => `timer ${ms} ${axis} ${Fmt.num(deg)}`,
  timers: () => "timers",
  ...
  fk: (a4) => `fk ${a4.map((d) => Fmt.num(d)).join(" ")}`,
  sleep: () => "sleep",
  wake: () => "wake",
...
const Parse = {
  ...
  line(text) {
    ...
    if ((m = t.match(this.RX_APPLIED))) {
      const sm = parseFloat(m[1]);
      const name = sm >= 1.4 ? "FAST (150%)" : sm <= 0.6 ? "SLOW (50%)" : "NORMAL (100%)";
      return { type: "profile", name };
    }
    if ((m = t.match(this.RX_POS))) {
      return { type: "pos", deg: [+m[1], +m[2], +m[3], +m[4]] }
    }
...
const COMMAND_REF = [
  { cmd: "home", args: "", desc: "هوم هوشمند همه محورها (به ترتیب J1 تا J4)", cat: "پایه", chip: true },
  { cmd: "home", args: "<1-4>", desc: "هوم هوشمند فقط یک محور", cat: "پایه", chip: false },
  { cmd: "status", args: "", desc: "نمایش کامل وضعیت سیستم و ۴ محور", cat: "پایه", chip: true },
  { cmd: "enable", args: "", desc: "فعال‌سازی برق همه موتورها", cat: "پایه", chip: true },
  { cmd: "enable", args: "<1-4>", desc: "فعال‌سازی یک محور", cat: "پایه", chip: false },
  { cmd: "disable", args: "", desc: "قطع برق همه موتورها", cat: "پایه", chip: true },
  { cmd: "disable", args: "<1-4>", desc: "قطع برق یک محور", cat: "پایه", chip: false },
  ...
  { cmd: "moveall", args: "<d1>..<d5>", desc: "حرکت همزمان هر ۴ محور با زاویه (درجه)", cat: "حرکت", chip: false },
  ...
  { cmd: "fk", args: "<a1>..<a4>", desc: "کینماتیک مستقیم: زوایا → موقعیت XYZ", cat: "کینماتیک", chip: false },
