 ...
 * بتوانی کل GUI را بدون سخت‌افزار تست کنی.
 * ============================================================ */
"use strict";
...
class SimFirmware {
  ...
  moveAll(steps4) { for (let i = 0; i < FW.NUM_AXES; i++) this.moveToAxis(i, steps4[i]); }

  degToSteps(i, deg) { return Math.round(deg * FW.AXES[i].stepsPerDeg); }

  /* v1.0.36: UI-sync channel - identical to the firmware */
  emitPosFromSteps(steps4) {
    const d = steps4.map((st, i) => (st / FW.AXES[i].stepsPerDeg).toFixed(1));
  ...
  _tickHoming(now) {
    ...
    if (hp.phase === "seek") {
      ...
      /* برگشت دقیقاً به اندازهِ backoff — مرجع صفر = جایگاه پارک */
      ...
      if (Math.abs(step) >= Math.abs(diff)) {
        ...
        /* آفست نقطه‌ی صفر (Config.h: HOMING_ZERO_OFFSET_DEG) — برای J4 (گریپر) = ۹۰°.
  ...
  _tickDemo(now) {
    ...
    const demoSteps = pose.map((d, i) => this.degToSteps(i, d));
    this.moveAll(demoSteps);
    this.emitPosFromSteps(demoSteps);
    this.lastDemoAt = now;
  ...
  _dispatch(raw) {
    ...
    if (lower === "moveall") { this.emit("Format: moveall <d1> <d2> <d3> <d4>"); return; }
    ...
    if (lower.startsWith("fk ")) {
      ...
      for (let i = 0; i < FW.NUM_AXES; i++) angles[i] = parseFloat(parts[i]) || 0;
