...
const AXCOLORS = ["#22d3ee", "#a78bfa", "#fb923c", "#f472b6"];
...
const BOARD_MARKERS = /AXIS-3 Firmware|System Status|>>\s*POS|State:|Homed:|Unknown command|System initialized/i;
...
function rxLine(line, fromSim = false) {
  ...
  /* ---------- کنسول: فقط چیزی که خودِ کاربر خواسته ----------
     ...
     ثانیه با یک بلوکِ دوازده‌خطی و سه خط POS پر می‌شد و دستوراتی که خودِ
  ...
  /* وقتی هومینگ تمام شد، اسلایدرِ جوینت‌های هوم‌شده فوراً صفر شود —
     بدون معطلی برای poll بعدی. (J4 هم صفر است: آفست ۹۰ درجه‌اش همان‌جا
...
function disablePosIfUnsupported() {
  ...
  const h = $("portHint");
  if (h) h.innerHTML = `<b style="color:#ff9b9e">⚠ فریم‌ورِ برد «pos» ندارد</b> — برای اینکه اسلایدرها موقعیت واقعی برد را دنبال کنند و آفست صفرِ گریپر (J4) اعمال شود، <b>firmware/RobotArm_Firmware/</b> را روی Mega فلش کن و دوباره وصل شو.`;
  return true;
}
 ...
 * L2+L3، چون گریپر صفر نگه داشته می‌شود و ابزار در امتداد ساعد است.
...
function gotoGeom() {
  ...
  const L2e = L2 + L3;                       /* ساعد مؤثر (گریپر صفر) */
...
function renderStats() {
  $("statHomed").textContent = S.axes.filter((a) => a.homed).length + "/" + FW.NUM_AXES;
  $("statMoving").textContent = S.axes.filter((a) => a.moving).length;
  $("statEnabled").textContent = S.axes.filter((a) => a.enabled).length + "/" + FW.NUM_AXES;
...
function buildJoints() {
  ...
  FW.AXES.forEach((ax, i) => {
    ...
    /* آفست نقطه‌ی صفر این جوینت (Config.h: HOMING_ZERO_OFFSET_DEG).
       برای J4 (گریپر) = ۹۰ درجه: بعد از هوم، ۹۰° جلو می‌رود و آنجا صفر می‌شود. */
    ...
    /* عدد آبیِ «jCur» از این ردیف حذف شد: همیشه صفر نشان می‌داد و تکراریِ
       کارت‌های J1..J4 بود. موقعیتِ زنده را هم خودِ اسلایدر (کانال POS)
...
function buildHelp() {
  ...
  FW.AXES.forEach((ax) => {
    ...
    tr.innerHTML = `
      ...
      <td><code>${ax.pins.step}/${ax.pins.dir}/${ax.pins.enable}/${ax.pins.endstop}${ax.pins.step2 ? ` + M2: ${ax.pins.step2}/${ax.pins.dir2}/${ax.pins.enable2} (آینه‌ای)` : ""}</code></td>`;
    ab.appendChild(tr);
  });
}
...
function linkDiagnosis() {
  ...
  if (S.serial.rxCount === 0) {
    ...
    addConsole("warn", "   → ۲) ModemManager یا brltty پورت را قاپیده‌اند (روی چیپ CH340 رایج است). یک بار برای همیشه: bash <(curl -fsSL https://raw.githubusercontent.com/Draxx143/arm-3-axis/main/tools/fix-serial-port-ownership.sh) و بعد کابل را دوباره بزن.");
...
function init() {
  ...
  addConsole("sys", "[SYS] کنترل‌پنل بازوی رباتیک ۴ محوره بارگذاری شد — Esc = ESTOP");
