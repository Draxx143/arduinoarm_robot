...
public:
    // Constructor
    // موتور دوم (اختیاری): برای محوری که دو استپر روبروی هم دارد
    // (جوینت ۳). هر دو موتور یک STEP می‌گیرند؛ اگر mirrorDir2 درست
    // باشد، DIR موتور دوم همیشه برعکس موتور اول است. 255 = بدون موتور دوم.
    Axis(uint8_t stepPin, uint8_t dirPin, uint8_t enablePin,
         ...
         uint8_t stepPin2 = 255, uint8_t dirPin2 = 255,
         uint8_t enablePin2 = 255, bool mirrorDir2 = false);
    ...
    // موتور دوم دارد؟ (جوینت ۳: دو استپر روبروی هم)
    bool hasSecondMotor() const { return _stepPort2 != nullptr; }
...
private:
    ...
    // ---- موتور دوم (اختیاری؛ 255 = ندارد) ----
    uint8_t _stepPin2;
    uint8_t _dirPin2;
    uint8_t _enablePin2;
    bool _mirrorDir2;                // روبروی هم بودن: DIR برعکس
    volatile uint8_t* _stepPort2;
    uint8_t _stepMask2;
    volatile uint8_t* _dirPort2;
    uint8_t _dirMask2;
    volatile uint8_t* _enablePort2;
    uint8_t _enableMask2;
