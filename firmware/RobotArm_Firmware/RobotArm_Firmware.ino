...
/*
 * 3+1 DOF Robot Arm Firmware — TEST MODE (بدون ROS)
 *
 ...
 *   traj line <d1..d4> <ms>  -> حرکت هماهنگ همه‌ی محورها در زمان مشخص
...
// محدوده درجه هر محور: J1 پایه، J2 شانه، J3 آرنج (دو موتوره)، J4 گریپر
const float AXIS_MIN_DEG[NUM_AXES] = {
    -110.0, 0.0, 0.0, -90.0
};
const float AXIS_MAX_DEG[NUM_AXES] = {
    110.0, 100.0, 55.0, 90.0
};
...
void setup() {
    ...
    delay(500);
    Serial.println(F("======================================"));
    Serial.println(F("3+1 DOF Robot Arm - TEST MODE (No ROS)"));
    Serial.println(F("AXIS-3 Firmware v" FIRMWARE_VERSION));
    Serial.println(F("======================================"));
    ...
    Serial.println(F("  home, home <1-4>     - Smart homing"));
    ...
    Serial.println(F("  traj line <d1> <d2> <d3> <d4> <ms>"));
    Serial.println(F("  ik <x> <y> <z> / fk <a1..a4>"));
...
void handleSerialCommands() {
    if (Serial.available() > 0) {
        ...
        // ==================== Basic Commands ====================
        if (command == F("home")) {
            Serial.println(F("Starting homing of ALL joints (priority order J1 -> J4)..."));
        ...
        // ---- ترتیب اولویت هومینگ ----
            ...
            Serial.println(F(">> Change with: homeorder 1 2 3 4"));
        }
            ...
            Serial.println(F("Format: moveall <d1> <d2> <d3> <d4>"));
        }
        ...
        // ==================== Timer ====================
            ...
            if (secondSpace == -1) {
                Serial.println(F("Format: timer <ms> <axis 1-4> <degrees>"));
            } else {
        ...
        else if (command == F("move"))      { Serial.println(F("Format: move <axis 1-4> <steps>")); }
        else if (command == F("deg"))       { Serial.println(F("Format: deg <axis 1-4> <degrees>")); }
        else if (command == F("maxspeed"))  { Serial.println(F("Format: maxspeed <axis 1-4> <steps/s>")); }
        else if (command == F("accel"))     { Serial.println(F("Format: accel <axis 1-4> <steps/s2>")); }
        else if (command == F("homespeed")) { Serial.println(F("Format: homespeed <axis 1-4> <steps/s>")); }
        else if (command == F("timer"))     { Serial.println(F("Format: timer <ms> <axis 1-4> <degrees>")); }
        else if (command == F("ik"))        { Serial.println(F("Format: ik <x> <y> <z>")); }
        else if (command == F("fk"))        { Serial.println(F("Format: fk <a1> <a2> <a3> <a4>")); }
        else if (command == F("traj"))      { Serial.println(F("Format: traj line <d1> <d2> <d3> <d4> <ms> | traj stop")); }
...
void handleAxisValueCommand(String command, int prefixLen, int kind) {
    ...
    if (sp == -1) {
        Serial.println(F("Format: <cmd> <axis 1-4> <value>"));
        return;
    }
...
void printUsage() {
    ...
    Serial.println(F("  home / home <1-4> / abort / homeorder <j1..j4>"));
    Serial.println(F("  move <ax> <steps> / deg <ax> <deg> / moveall <d1..d4>"));
    Serial.println(F("  traj line <d1..d4> <ms> / traj stop"));
...
// نمونه: homeorder 1 2 3 4   -> اول جوینت ۱، بعد ۲، ۳، ۴ و ۵
// نمونه: homeorder 3 2 1 4   -> اول Z (اگر مثلاً بخوای Z اول بالا بره)
void handleHomingOrderCommand(String command) {
    ...
    if (count != NUM_AXES) {
        ...
        Serial.print(count);
        Serial.println(F(". Example: homeorder 1 2 3 4"));
        return;
    }
...
void handleTrajLineCommand(String command) {
    ...
    for (int i = 0; i < NUM_AXES + 1; i++) {
        ...
        if (token.length() == 0) {
            Serial.println(F("Format: traj line <d1> <d2> <d3> <d4> <ms>"));
            return;
        }
        ...
        if (spacePos == -1) {
            if (i < NUM_AXES) {
                Serial.println(F("!! Not enough values (need 4 angles + duration)"));
                return;
            }
            break;
        }
