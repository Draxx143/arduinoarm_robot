...
Axis::Axis(uint8_t stepPin, uint8_t dirPin, uint8_t enablePin,
           ...
           uint8_t enablePin2, bool mirrorDir2)
    : _stepPin(stepPin), _dirPin(dirPin), _enablePin(enablePin),
      _endstopPin(endstopPin),
      _stepPin2(stepPin2), _dirPin2(dirPin2),
      _enablePin2(enablePin2), _mirrorDir2(mirrorDir2),
    ...
    _stepPort2   = nullptr;
    _dirPort2    = nullptr;
    _enablePort2 = nullptr;
    _stepMask2 = _dirMask2 = _enableMask2 = 0;
...
void Axis::init() {
    ...
    // موتور دوم (جوینت ۳): اگر پین داده شده، راه‌اندازی می‌شود؛
    // انداستاپ مشترک است و جداگانه ندارد
    if (_stepPin2 != 255 && _dirPin2 != 255 && _enablePin2 != 255) {
        pinMode(_stepPin2, OUTPUT);
        pinMode(_dirPin2, OUTPUT);
        pinMode(_enablePin2, OUTPUT);
        _stepPort2   = portOutputRegister(digitalPinToPort(_stepPin2));
        _stepMask2   = digitalPinToBitMask(_stepPin2);
        _dirPort2    = portOutputRegister(digitalPinToPort(_dirPin2));
        _dirMask2    = digitalPinToBitMask(_dirPin2);
        _enablePort2 = portOutputRegister(digitalPinToPort(_enablePin2));
        _enableMask2 = digitalPinToBitMask(_enablePin2);
        *_stepPort2   &= ~_stepMask2;
        *_enablePort2 |= _enableMask2;    // HIGH = موتور غیرفعال
    }
...
void Axis::enableMotor() {
    ...
    if (_enablePort2) *_enablePort2 &= ~_enableMask2;
    _enabled = true;
}
...
void Axis::writeDirection(int8_t sign) {
    ...
    // موتور دوم روبروی موتور اول است → DIR برعکس تا هم‌جهت بچرخند
    if (_dirPort2) {
        bool high2 = _mirrorDir2 ? !high : high;
        if (high2) *_dirPort2 |= _dirMask2;
        else       *_dirPort2 &= ~_dirMask2;
    }
}
...
bool Axis::tick() {
    ...
    // هر دو موتور جوینت ۳ دقیقاً یک پالس می‌گیرند (هم‌زمان می‌چرخند)
    *_stepPort |= _stepMask;
    if (_stepPort2) *_stepPort2 |= _stepMask2;
